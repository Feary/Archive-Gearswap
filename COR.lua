...
function get_sets()
	...
	-- Ammo
		-- Quick Draw Only
		QDBullet = {""}
		QDBullet.Feary = "Animikii Bullet"
		QDBullet.Derion = "Animikii Bullet"
		-- Normal TP Bullets
		TPBullet = {}
		TPBullet.Feary = "Eminent Bullet"
		TPBullet.Derion = "Eminent Bullet"
		-- WS Bullets
		WSBullet = {}
		WSBullet.Feary = "Adlivun Bullet"
		WSBullet.Derion = "Eminent Bullet"
		-- Magical WS Bullets "use the cheapest"
		MBullet = {}
		MBullet.Feary = "Bullet"
		MBullet.Derion = "Bullet"
		
	-- Luzaf's Ring - 1 to uses
		luzaf = 1
...
--  Lock MDT
	...
	elseif command == "sws" then
		if WS == "Leaden Salute" then
			WS = "Wildfire"
			windower.add_to_chat(121,'WS: Wildfire')
		elseif WS == "Wildfire" then
			WS = "Last Stand"
			windower.add_to_chat(121,'WS: Last Stand')
		elseif WS == "Last Stand" then
			WS = "Leaden Salute"
			windower.add_to_chat(121,'WS: Leaden Salute')
		end
	end
end
...
-- Range Attack 
    if spell.name == 'Ranged' then
		...
		if QDBullets:contains(player.equipment.ammo) then
			...
			equip({ammo=TPBullet[player.name]})
			cancel_spell()
		else
			if player.equipment.ammo == TPBullet[player.name] then
				...
				equip({ammo=TPBullet[player.name]})
				windower.add_to_chat(121,'Equipping Ammo')
			end			
		end
-- Cor Rolls
	...
	elseif spell.type == "CorsairShot" then
		if spell.english:wcmatch('Dark Shot|Light Shot') then
			if spell.element == world.day_element or spell.element == world.weather_element or buffactive[elements.storm_of[spell.element]] then
				equip(sets.midcast.Nuke.MB.Acc, {ammo=QDBullet[player.name], waist="Hachirin-no-Obi"})
			else
				equip(sets.precast.QD.ACC, {ammo=QDBullet[player.name]})
			end
		else
			if spell.element == world.day_element or spell.element == world.weather_element or buffactive[elements.storm_of[spell.element]] then
				equip(sets.precast.QD.MAB, {ammo=QDBullet[player.name], waist="Hachirin-no-Obi"})
			else
				equip(sets.precast.QD.MAB,{ammo=QDBullet[player.name]})
			end
		end
-- JobAbility
					...
					if spell.target.distance <= ranged_weaponskills_Distance[spell.name] then
						if sets.precast.RAWS[spell.name] then
							if spell.name == "Wildfire" or spell.name == "Leaden Salute" then
								if spell.element == world.day_element or spell.element == world.weather_element or buffactive[elements.storm_of[spell.element]] then
									equip(sets.precast.RAWS[spell.name], {ammo=MBullet[player.name], waist="Hachirin-no-Obi"})
								else
									equip(sets.precast.RAWS[spell.name], {ammo=MBullet[player.name]})
								end
							else
