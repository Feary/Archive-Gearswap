...
function get_sets()
	...
	-- Define Default Values for Variables
		...
		WS = 'Savage Blade'
		-- Blue Spells 
		bluSpells = S{'Blank Gaze', 'Geist Wall', 'Jettatura', 'Soporific',
...
function self_command(command)
	...
	elseif command == 'TP' then
		-- WHen Locked in PDT or MDT Gear
...
function previous_set()
	...
	if player.equipment.sub == mshield then
		if buffactive["Cover"] then
			...
			elseif Mode == 1 then 
				...
				windower.add_to_chat(121,'Aegis - Cover - HP TP Set')		
			-- TP Mode
			elseif Mode == 2 then
				if TPMode == 0 then
			...
			elseif Mode >= 3 then
				-- Error checking. set back to default
				Mode = 0
				windower.add_to_chat(121,'Aegis - Cover - Acc TP Set')	
			end
		else
			...
			elseif Mode == 1 then 
				...
				windower.add_to_chat(121,'Aegis - HP TP Set')		
			-- TP Mode
			elseif Mode == 2 then
				if TPMode == 0 then
	...
	elseif player.equipment.sub == pshield then
		...
		if buffactive["Cover"] then
			if Mode == 0 then
				equip(sets.TP.Tank,{main={name=Sword, priority=2},sub={name=pshield,priority=1}},{body="Cab. Surcoat +1"})
			...
			elseif Mode == 1 then 
				...
				windower.add_to_chat(121,'Ochain - Cover - HP TP Set')
			-- TP Mode
			elseif Mode == 2 then
				if TPMode == 0 then
